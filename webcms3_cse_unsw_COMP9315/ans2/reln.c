...
void reInsertAndClearAPage(Reln r, FILE *f, PageID pid, Page pg)
{
	Count pnt = pageNTuples(pg), i;
	if (pnt == 0) return;
	char *c = pageData(pg);
	Tuple *pts = (Tuple *)malloc(pnt*sizeof(Tuple));
	for (i = 0; i < pnt; i++) {
		pts[i] = copyString(c);
		c += strlen(c) + 1;
	}
...
void splitPage(Reln r)
{
	...
	reInsertAndClearAPage(r, r->data, pid, pg);
	pg = getPage(r->data, pid);
	Page ovpg; PageID ovp = pageOvflow(pg);
	while (ovp != NO_PAGE) {
		ovpg = getPage(r->ovflow, ovp);
		reInsertAndClearAPage(r, r->ovflow, ovp, ovpg);
		ovp = pageOvflow(ovpg);
	}
	free(pg);
}
...
PageID addToRelation(Reln r, Tuple t)
{
	...
	if (addToPage(pg,t) == OK) {
		Count pnt = pageNTuples(pg) - 1;
		putPage(r->data,p,pg);
		r->ntups++;
		if (pnt != 0 && pnt % c == 0) splitPage(r);
		return p;
	}
